...
public class TimeSeriesDataCacheTest {
	List<OrderBook> orderBooks1 = new ArrayList<>();
	List<OrderBook> orderBooks2 = new ArrayList<>();
	
	TimeSeriesDataCache<OrderBook> cache = new TimeSeriesDataCache<OrderBook>();
	...
	public void testContainsKey() {
		cache.putAll(sec1, orderBooks1);
		cache.putAll(sec2, orderBooks2);
		assertEquals(true, cache.containsKey(sec1));
		assertEquals(true, cache.containsKey(sec2));
		
		cache.remove(sec1);
		assertEquals(false, cache.containsKey(sec1));
	}
	...
	public void testGetPrevious() {
		...
		OrderBook ob = cache.getPrevious(sec1);
		assertEquals(cache.get(sec1).get(3), ob);
		assertNull(cache.getPrevious(sec2));

		cache.clear();
		assertNull(cache.getPrevious(sec1));
		cache.put(sec1, orderBooks1.get(0));
		cache.put(sec1, orderBooks1.get(1));
		assertEquals(orderBooks1.get(0), cache.getPrevious(sec1));
	}
}
