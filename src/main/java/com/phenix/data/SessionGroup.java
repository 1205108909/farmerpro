...
public class SessionGroup implements Comparable<SessionGroup> {
	...
	public int compareTo(SessionGroup o) {
		throw new UnsupportedOperationException(SessionGroup.class.getSimpleName() + " is not allowed to be compare");
	}

	/**
	 * TODO: further enhance
	 * Make sure the input TradingSessions are invalid for the following two aspects,
	 * 1, they cover all the time continuously, which means the first session's end time == next session's start time
	 * 2. the very first session starts before 9:00 and last session ends after 15:00, in order to cover all possible data.
	 * @param sessions
	 * @param name
	 */
	public static void validateTradingSessions(List<TradingSession> sessions, String name) {
		int size = sessions.size();
		if (size == 0 || size == 1) {
			throw new IllegalDataException("SessionGroup " + name + " contains " + size + " TradingSessions.");
		} else {
			for (int i = 0; i < size - 1; i ++) {
				if (!sessions.get(i).getEndTime().equals(sessions.get(i + 1).getStartTime())) {
					throw new IllegalDataException("SessionGroup " + name + " Session " + i + " is not continuous with the next one, may lead to error.");
				}
			}
			LocalDateTime firstSessionStartAt = sessions.get(0).getStartTime();
			LocalDateTime lastSessionEndAt = sessions.get(size - 1).getEndTime();
			if (firstSessionStartAt.getHour() >= 9 || lastSessionEndAt.getHour() < 15) {
				throw new IllegalDataException("SessionGroup " + name + " may not cover all trading time, plz check.");
			}
		}
	}
}
