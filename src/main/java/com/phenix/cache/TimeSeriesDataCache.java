...
public class TimeSeriesDataCache<T extends ITimeable> extends EngineCache<Security, List<T>> {
	@Getter
	...
	public T getPrevious(Security sec_) {
		List<T> cached = super.get(sec_);
		if (cached == null || cached.size() <= 1)
			return null;
		else
			return cached.get(cached.size() - 2);
	}
	...
	public T getLastBeforeOrEqual(Security sec_, LocalDateTime t_) {
		T t = null;
		...
		if (cached == null || cached.size() == 0)
			...
			// for general purpose, a binary search should have worse performance
			for (int i = cached.size() - 1; i >= 0; i--) {
				if (!cached.get(i).getDateTime().isAfter(t_)) {
					t = cached.get(i);
					break;
				}
			}
	...
	public T getLastBeforeOrEqual(Security sec_, ITimeable t_) {
		return getLastBeforeOrEqual(sec_, t_.getDateTime());
	} 
}
